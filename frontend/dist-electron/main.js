...
function requireNodeExternalApi() {
  ...
  class NodeExternalApi {
    ...
    getAppName() {
      var _a;
      return this.appName || ((_a = this.getAppPackageJson()) == null ? void 0 : _a.name);
    }
    /**
     * @private
     * @returns {undefined}
...
function requireElectronExternalApi() {
  ...
  class ElectronExternalApi extends NodeExternalApi {
    ...
    getAppName() {
      ...
      let appName;
      try {
        appName = ((_a = this.electron.app) == null ? void 0 : _a.name) || ((_b = this.electron.app) == null ? void 0 : _b.getName());
      } catch {
      }
  ...
  class EventLogger {
    ...
    handleEvent({ eventName, eventSource, handlerArgs }) {
      ...
      if (log) {
        ...
        (_a = logFns == null ? void 0 : logFns[this.level]) == null ? void 0 : _a.call(logFns, ...log);
      }
    }
  }
...
function getScriptPath(scriptName) {
  const scriptPath = require$$1.platform() === "win32" ? `${process.cwd()}/resources` : `${process.platform}-unpacked/resources`;
  return scriptPath;
}
const initBackgroundProcesses = () => {
  Logger.info("Initializing background processes");
  ["processOne", "processTwo"].forEach((fileName) => {
    try {
      const childProcess = require$$0$1.execFile(
        "python3",
...
const fullDate = `${day < 10 ? "0" + day : day}-${month < 10 ? "0" + month : month}-${year}-${hours < 10 ? "0" + hours : hours}-${minutes < 10 ? "0" + minutes : minutes}`;
Logger.transports.file.resolvePathFn = () => path.join(
  `${process.env.HOME}/Workout-Training-Admin/frontend/electron/logs`,
  `logs-${fullDate}.log`
);
process.env.DIST = path.join(__dirname, "../dist");
process.env.VITE_PUBLIC = require$$0$5.app.isPackaged ? process.env.DIST : path.join(process.env.DIST, "../public");
let win;
